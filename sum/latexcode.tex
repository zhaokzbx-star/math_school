...
% 题目格式
\newcommand{\titleformat}[1]{\begin{center}\fontsize{16pt}{18pt}\bfseries #1\end{center}}

% 一级标题格式
\newcommand{\sectionformat}[1]{\begin{center}\fontsize{14pt}{16pt}\bfseries \thesection\quad #1\end{center}}

% 二级标题格式
\newcommand{\subsectionformat}[1]{\fontsize{12pt}{14pt}\bfseries \thesubsection\quad #1}

% 三级标题格式
\newcommand{\subsubsectionformat}[1]{\fontsize{12pt}{14pt}\bfseries \thesubsubsection\quad #1}
...
\begin{center}
{\fontsize{16pt}{18pt}\bfseries 城市极端降雨下内涝韧性提升与疏散路径优化}
\end{center}

\vspace{1cm}

{\fontsize{12pt}{14pt}\bfseries 摘要}
...
\textbf{问题一}，基于水量平衡原理，构建引入管网排水折减系数$C_d$与街区汇水乘数$\theta$的一维离散时间积水动态演化模型，刻画极端降雨下管网排水能力衰减与下垫面汇流放大效应；通过三次样条插值实现降雨序列逐分钟细化，并利用分段函数建立积水深度与通行能力系数、安全度的映射关系。采用最小二乘法反演标定参数，得到$C_d = 0.100$、$\theta = 12.05$，模型拟合决定系数$R^2 = 0.947$，均方根误差仅5.631 cm，预测精度满足应急决策需求。结果显示，L5路段积水峰值达36.4 cm，60分钟时通行能力完全丧失，为片区内涝最高风险节点。
...
{\fontsize{12pt}{14pt}\songti
原始降雨数据以5分钟为时间间隔给出，为获得连续时间尺度下的降雨输入，采用三次样条插值方法对降雨序列进行平滑处理，将其转换为逐分钟降雨强度$I_{\text{rain}}$（m/min）。
}

\subsubsectionformat{模型建立}

{\fontsize{12pt}{14pt}\songti
考虑到复杂的二维或三维水动力模型计算耗时较长、基础数据要求高，往往不适用于应急防汛情况下的快速推演\cite{Zhang2022}。因此，本文参考了学术界关于城市内涝多因素耦合的简化处理思路，基于地表水量平衡与一维管网排水机理，建立路段的积水深度动态演化方程\cite{Zhang2022}：
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a1/answer1图一_积水深度动态演化.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a1/answer1图二_拟合散点图.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a1/answer1图三_通行能力热力图.png}
    \caption{各路段通行能力热力图}
...
{\fontsize{12pt}{14pt}\songti
\textbf{系统层评价模型（MC-EWM-TOPSIS）}\cite{Doubao}：
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a2/answer2图一_权重对比图.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a2/answer2图二_韧性诊断雷达图.png}
    \caption{系统韧性诊断雷达图}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a2/answer2图四_薄弱路段识别图.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a2/answer2图三_薄弱环节识别图.png}
    \caption{各指标敏感度系数排名图}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a3/answer3图一_路网拓扑与最优路径.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a3/answer3图二_积水深度动态演变.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a3/answer3图三_疏散方案对比.png}
...
动态时空规划算法相比静态导航方案，耗时缩短约\textbf{68\%}，安全度提升\textbf{10.8倍}，有效验证了动态路径规划在应对城市内涝应急疏散中的优越性。问题四将在此基础上进一步优化排涝工程方案，实现“路径优化—工程优化”的逻辑递进。
}
...
{\fontsize{12pt}{14pt}\songti
\textbf{求解流程}：采用“蒙特卡洛采样—帕累托筛选—K-Means聚类”三阶段算法。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a4/answer4图一_帕累托前沿.png}
    \caption{多目标优化帕累托前沿图}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{a4/answer4图二_成本效益曲线.png}
    \caption{成本效益曲线与性价比拐点图}
...
{\fontsize{12pt}{14pt}\songti
本研究通过物理机理建模、数据驱动优化与多目标决策方法，构建了城市内涝韧性分析模型体系。积水预测模型标定得到管网排水折减系数 $C_d = 0.100$、街区汇水乘数 $\theta = 12.05$，模型 $R^2 = 0.947$ 表明预测精度较高。系统韧性综合评分为0.4308，处于中等水平，其中绿色调蓄容积是影响韧性的最关键因素。动态时空规划算法相比静态导航方案，疏散效率提升68%、安全度提升10.8倍。多目标优化成功生成三个典型方案：保守防御型（成本1225万元）、均衡拐点型（成本2984万元）和激进海绵型（成本4569万元），分别适用于预算有限、追求平衡和最高韧性等不同场景。
...
% ==========================================
\clearpage
\fontsize{12pt}{14pt}\bfseries 参考文献
